...
class PersistentDataHelper(context: Context) {
    ...
    private fun cursorToPoint(cursor: Cursor): Point {
        val point = Point()
        point.x = cursor.getFloat(DbConstants.Points.Columns.coord_x.ordinal).toInt()
        point.y = cursor.getFloat(DbConstants.Points.Columns.coord_y.ordinal).toInt()
        return point
    }
    ...
    private fun cursorToLine(cursor: Cursor): Line {
        ...
        startPoint.x = cursor.getFloat(DbConstants.Lines.Columns.start_x.ordinal).toInt()
        startPoint.y = cursor.getFloat(DbConstants.Lines.Columns.start_y.ordinal).toInt()
        line.start = startPoint
        val endPoint = Point()
        endPoint.x = cursor.getFloat(DbConstants.Lines.Columns.end_x.ordinal).toInt()
        endPoint.y = cursor.getFloat(DbConstants.Lines.Columns.end_y.ordinal).toInt()
        line.end = endPoint
        return line
    }
